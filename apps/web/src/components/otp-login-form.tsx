...
export function OtpLoginForm({ nextPath }: { nextPath: string }) {
  ...
  const [message, setMessage] = useState("Sign in with your phone to continue.");

  async function requestCode(): Promise<string | null> {
    const receipt = await createWebApiClient().requestOtp({ phone, purpose: "LOGIN" });
    setDebugCode(receipt.debugCode ?? null);
    return receipt.debugCode ?? null;
  }

  async function verifyWith(otp: string): Promise<void> {
    const result = await createWebApiClient().verifyOtp({ phone, code: otp, purpose: "LOGIN" });
  ...
  return (
    <Surface state={state} title="Member sign in">
      <p>{message}</p>
      <form
        className="stack"
        onSubmit={(event) => {
