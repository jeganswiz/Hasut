...
export function storyVisualState(story: StoryView): "image" | "preparing" | "video" {
  if (story.kind === "IMAGE") {
    return "image";
  }
  return story.playbackStatus === "PENDING" ? "preparing" : "video";
}

/** Already-absolute playback. Relative `/media/hls` paths go through `resolveMediaUrl`. */
export function reachablePlaybackUrl(url: string | null): url is string {
  if (url === null) {
    return false;
  }
